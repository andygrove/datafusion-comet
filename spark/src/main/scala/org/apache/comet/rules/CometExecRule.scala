...
import org.apache.comet.serde.{CometOperatorHandler, Compatible, Incompatible, OperatorOuterClass, QueryPlanSerde, Unsupported}
...
case class CometExecRule(session: SparkSession) extends Rule[SparkPlan] {
  ...
  private def transform(plan: SparkPlan): SparkPlan = {
    ...
    def newSinkWithProto(op: SparkPlan): SparkPlan = {
      ...
      case plan if plan.children.exists(_.isInstanceOf[BroadcastExchangeExec]) =>
        val newChildren = plan.children.map {
          case b: BroadcastExchangeExec
            ...
            CometExecRule.operator2Proto(b) match {
              case Some(nativeOp) =>
      ...
      case s: ShuffleExchangeExec =>
        ...
        val nativeOrColumnarShuffle = if (nativeShuffle.isDefined) {
          ...
          if (columnarShuffleSupported(s)) {
            val newOp = CometExecRule.operator2Proto(s)
            newOp match {
              case Some(nativeOp) =>
                s.child match {
...
object CometExecRule extends Logging {
