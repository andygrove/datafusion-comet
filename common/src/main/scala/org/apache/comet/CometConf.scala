...
object CometConf extends ShimCometConf {
  ...
  val COMET_EXEC_SHUFFLE_COMPRESSION_CODEC: ConfigEntry[String] = conf(
    ...
    .doc(
      "The codec of Comet native shuffle used to compress shuffle data. Only lz4 and zstd are " +
        "supported. Compression can be disabled by setting spark.shuffle.compress=false.")
    .stringConf
