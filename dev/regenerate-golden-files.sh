...
#
# Options:
#   --spark-version <version>  Only regenerate for specified Spark version (3.4, 3.5, 4.0, or 4.2)
...
main() {
    ...
    while [[ $# -gt 0 ]]; do
        case $1 in
            ...
            -h|--help)
                ...
                echo ""
                echo "Options:"
                echo "  --spark-version <version>  Only regenerate for specified Spark version (3.4, 3.5, 4.0, or 4.2)"
    ...
    if [ -n "$target_version" ]; then
        if [[ ! "$target_version" =~ ^(3\.4|3\.5|4\.0|4\.2)$ ]]; then
            echo "[ERROR] Invalid Spark version: $target_version"
            echo "[ERROR] Supported versions: 3.4, 3.5, 4.0, 4.2"
            exit 1
        fi
    fi
